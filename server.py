...
# Ensure Trellis is in python path (FINAL FIX)
# We must use sys.path.insert(0, ...) to prioritize local TRELLIS modules
# over potentially broken system installations (like spconv).
TRELLIS_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "TRELLIS")
sys.path.insert(0, TRELLIS_PATH) 

# Ensure the TRELLIS/spconv directory is also prioritized, as it holds the compiled core.
SPCONV_PATH = os.path.join(TRELLIS_PATH, "spconv")
if os.path.isdir(SPCONV_PATH):
    sys.path.insert(0, SPCONV_PATH)
...
async def lifespan(app: FastAPI):
    ...
    try:
        # CRITICAL FIX: Import spconv and TrellisImageTo3DPipeline here.
        # This order (spconv first) is necessary to ensure Pybind11 registers
        # the required C++ types (tv::Tensor) before the pipeline initializes.
